...
export default function AdminPage() {
  ...
  return (
    <main className={`min-h-screen ${bg} flex flex-col transition-colors duration-300`}>
      ...
      {/* Notificación */}
      {notification && (
        <div className="fixed bottom-6 right-6 z-50 bg-[#1c1c1e] text-white text-xs px-4 py-3 rounded-xl flex items-center gap-3 shadow-lg">
      ...
      {/* Topbar */}
      <div className={`flex items-center justify-between px-5 py-3 ${sidebarBg} border-b ${borderColor} transition-colors duration-300`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => router.push('/admin/dashboard')} className={`text-xs px-3 py-1.5 rounded-lg border transition-colors ${btnBorder}`}>
            Dashboard
          </button>
          {profile?.role === 'admin' && (
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {selected ? (
          <div className={`flex-1 flex flex-col overflow-hidden transition-all duration-200 ${panelVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-2'}`}>
            ...
            {/* Hero */}
            <div className={`${panelBg} border-b ${borderColor} px-6 py-4 border-l-4 ${accentBorder(selected.status)} transition-colors duration-500`}>
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h2 className={`text-xl font-medium tracking-tight capitalize ${textPrimary}`}>
            ...
            {/* Scroll area */}
            <div className={`flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-3 ${bg}`}>
                ...
                {documents.length === 0 ? (
                  ...
                  documents.map((doc, i) => (
                    <div key={doc.id} className={`flex items-center gap-3 px-4 py-2.5 ${hoverBg} transition-colors ${i > 0 ? `border-t ${borderColor}` : ''}`}>
                      ...
                      <button onClick={() => getDownloadUrl(doc.file_path)} className={`text-[10px] font-medium transition-colors ${accentText(selected.status)} hover:opacity-70`}>View</button>
            ...
            {/* Footer actions */}
            {!showArchive && (
              <div className={`${panelBg} border-t ${borderColor} px-6 py-3 flex items-center gap-2 transition-colors duration-300`}>
                <button onClick={() => updateStatus('pending')} className={`px-4 py-2 text-xs border rounded-lg transition-colors ${btnBorder}`}>
                  Mark as pending
                </button>
