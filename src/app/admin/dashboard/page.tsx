  ...
  const init = async () => {
    ...
    if (clients) {
      setStats({
        ...
        pending: clients.filter(c => c.status === 'pending' && !c.archived).length,
        in_progress: clients.filter(c => c.status === 'in_progress' && !c.archived).length,
        complete: clients.filter(c => c.status === 'complete' && !c.archived).length,
        archived: clients.filter(c => c.archived).length,
        documents: documents?.length || 0
      })
      setRecent(clients.filter(c => !c.archived).slice(0, 5))
    }
    setLoading(false)
  }
    ...
    <main className={`min-h-screen ${bg} transition-colors duration-300`}>
      ...
      <div className={`flex items-center justify-between px-6 py-3 border-b ${darkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white'}`}>
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs ml-2 px-2 py-0.5 rounded-full ${darkMode ? 'bg-gray-700 text-gray-400' : 'bg-gray-100 text-gray-500'}`}>Dashboard</span>
        </div>
        <div className="flex items-center gap-3">
          <span className={`text-xs ${subtext}`}>{profileName}</span>
          <button
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        <div className="mb-8">
          <h1 className={`text-2xl font-semibold ${text}`}>Overview</h1>
          <p className={`text-sm ${subtext} mt-1`}>Here's what's happening with your clients.</p>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            { label: 'Active clients', value: stats.total, color: 'text-blue-600', bg: 'bg-blue-50' },
        ...
        <div className={`${card} border rounded-2xl p-6`}>
          <div className="flex items-center justify-between mb-4">
            <p className={`text-xs font-medium ${subtext} uppercase tracking-wider`}>Recent clients</p>
            <button onClick={() => router.push('/admin')} className="text-xs text-blue-500 hover:text-blue-700 transition">View all →</button>
          </div>
          {recent.length === 0 ? (
